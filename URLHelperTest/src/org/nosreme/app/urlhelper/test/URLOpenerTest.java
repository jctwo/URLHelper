...

{
	...
	public void testExpand1()
	{
		...
		FakeHttpOpener factory = new FakeHttpOpener("http://redirected.example.com/");
        ...
        try {
          method = activity.getClass().getDeclaredMethod("onActivityResult", int.class, int.class, Intent.class);
          method.setAccessible(true);
      	  method.invoke(activity, requestId, ActionChooser.RESULT_EXPAND, fakeResultIntent);
        } catch (SecurityException e) {
        	fail("Security exception");
        } catch (NoSuchMethodException e) {
        	fail("NoSuchMethodException");
        ...
        /* Check that it's taken the right action, ie expand and re-launch */
        sent = getStartedActivityIntent();
        assertNotNull(sent);
        assertEquals(sent.getDataString(), "http://redirected.example.com/");
  	}
